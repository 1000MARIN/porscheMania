          ...
          <div class="row">
            <div class="col s12" style="padding: 3% 2%;">
				  ...
				  <tbody>
	                ...
	                if (pageDTO.getTotalCount() > 0) {
	                	...
	                	for (MemberVO memberVO : memberList) {
	                		...
	                		%>
				    <tr class="table-light" onclick="location.href='/pmania/member/memberContent.jsp?id=<%=memberVO.getId() %>&pageNum=<%=pageDTO.getCri2().getPageNum() %>'">
              ...
              if (pageDTO.isPrev()) {
            	  %>
                  <li class="page-item">
                  	<a class="page-link" href="/pmania/member/memberList.jsp?pageNum=<%=pageDTO.getStartPage() - 1 %>&type=<%=pageDTO.getCri2().getType() %>&keyword=<%=pageDTO.getCri2().getKeyword() %>#board" data-abc="true"><i class="fa fa-angle-left"></i></a>
                  </li>
                  <%
              }
              %>
              ...
              for (int i=pageDTO.getStartPage(); i<=pageDTO.getEndPage(); i++) {
                  ...
                  <li class="page-item <%=(pageDTO.getCri2().getPageNum() == i) ? "active" : "" %>">
                  	<a class="page-link" href="/pmania/member/memberList.jsp?pageNum=<%=i %>&type=<%=pageDTO.getCri2().getType() %>&keyword=<%=pageDTO.getCri2().getKeyword() %>#board" data-abc="true"><%=i %></a>
                  </li>
                   <%
              }
              %>
              ...
              if (pageDTO.isNext()) {
                  ...
                  <li class="page-item">
                  	<a class="page-link" href="/pmania/member/memberList.jsp?pageNum=<%=pageDTO.getEndPage() + 1 %>&type=<%=pageDTO.getCri2().getType() %>&keyword=<%=pageDTO.getCri2().getKeyword() %>#board" data-abc="true"><i class="fa fa-angle-right"></i></a>
                  </li>
                  <%
              }
              %>
  ...
  <script>
    // 검색 버튼 클릭시
  	$('#btnSearch').on('click', function () {
  		...
  		location.href = '/pmania/member/memberList.jsp?' + query + '#board';
  	});
  </script>
